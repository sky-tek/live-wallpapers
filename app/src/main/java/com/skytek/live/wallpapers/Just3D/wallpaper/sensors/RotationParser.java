package com.skytek.live.wallpapers.Just3D.wallpaper.sensors;
...
public class RotationParser extends GenericParser {
    ...
    double[] parseRoatationMatrix(float[] rotationMatrix) {
        ...
        // Yahan Changes ki han roll mein 08/22/2021
        if (pitch < 70) {
            roll = -toDegrees(Math.atan2(-rotationMatrix[6], abs(rotationMatrix[3])));
        }
        else {
